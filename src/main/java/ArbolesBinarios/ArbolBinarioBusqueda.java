...
public class ArbolBinarioBusqueda<K extends Comparable<K>,V> implements IArbolBusqueda<K,V> {
    ...
    public ArbolBinarioBusqueda(List<K> clavesInOrden, List<V> valoresInOrden,
     ...
     private NodoBinario reconstruirConPreOrden(List<K> clavesInOrden, List<V> valoresInOrden,
         ...
         NodoBinario hijoIzquierdo=reconstruirConPreOrden(clavesInOrdenPorIzquierda, valoresInOrdenPorIzquierda,
         ...
         NodoBinario hijoDerecha=reconstruirConPreOrden(clavesInOrdenPorDerecha, valoresInOrdenPorDerecha,
                                                           ClavesPreOrdenPorDerecha, ValoresPreOrdenPorDerecha);        
         //Armando el nodoActual
         NodoBinario nodoactual=new NodoBinario(clavePadre,valorPadre);
         nodoactual.setHijoIzquierdo(hijoIzquierdo);
         nodoactual.setHijoDerecho(hijoDerecha);
         return nodoactual;
     }
     
     private NodoBinario reconstruirConPosOrden(List<K> clavesInOrden, List<V> valoresInOrden, 
         ...
         NodoBinario hijoIzquierdo=reconstruirConPosOrden(clavesInOrdenPorIzquierda, valoresInOrdenPorIzquierda,
                                                           ClavesPreOrdenPorIzquierda, ValoresPreOrdenPorIzquierda);
         //para armar rama derecha
         List<K> clavesInOrdenPorDerecha=clavesInOrden.subList(posicionClavePadreEnInOrden+1,clavesInOrden.size());
         List<V> valoresInOrdenPorDerecha=valoresInOrden.subList(posicionClavePadreEnInOrden+1,valoresInOrden.size());
         ...
         NodoBinario hijoDerecha=reconstruirConPosOrden(clavesInOrdenPorDerecha, valoresInOrdenPorDerecha,
                                                           ClavesPreOrdenPorDerecha, ValoresPreOrdenPorDerecha);        
         //Armando el nodoActual
         NodoBinario nodoactual=new NodoBinario(clavePadre,valorPadre);
         nodoactual.setHijoIzquierdo(hijoIzquierdo);
    ...
    public int size() {
        ...
        int cantNodos=0;
         Stack<NodoBinario> pilaNodos=new Stack<>();
        pilaNodos.push(raiz);
        while (!pilaNodos.isEmpty()){
            NodoBinario nodoactual=pilaNodos.pop();
            cantNodos++;
    ...
    private int alturaIterativo(){
        ...
        while (!colaNodos.isEmpty()) {            
            ...
            while (i<cantidadNodosCola) {                
                NodoBinario nodoactual=colaNodos.poll();
...
//     }
   ...
   public int cantidadenodosConsoloHijosIzquierdosRec(){
    ...
    public int cantidadNodosConSoloHIdespuesNivel(int NivelaBuscar){
        ...
        int alturaArbol=0;
        int contador=0;
        Queue<NodoBinario> colaNodos=new LinkedList<>();
        colaNodos.offer(raiz);
        while (!colaNodos.isEmpty()) {            
            int cantidadNodosCola=colaNodos.size();
            int i=0;
    ...
    public boolean arbolLleno(){
        ...
        if (!arbolesSimilars(raiz.getHijoIzquierdo(),raiz.getHijoDerecho())) {
                return false;
        }
        return true;
    }
